...
void MainWindow::on_SignIn_clicked()
{
    ...
    if (validLogin)
    {
        ...
        network->readPosts("../resources/posts.txt");
        ui->UserBack->hide();
        createMainFriends();
    }
}
...
void MainWindow::createUserFriends()
{
    ...
    for (size_t y = 0; y < currentUser->getFriends().size(); ++y)
    {
        ...
        {
            if (friends == currentUser->getFriends()[y])
                validFriend = false;
        }
        if (validFriend)
            ui->UserFriends->setItem(y, 2, new QTableWidgetItem(QString("Add Friend")));
        else
            ui->UserFriends->setItem(y, 2, new QTableWidgetItem(QString("Added")));
    }
